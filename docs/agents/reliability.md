...
Memory retrieval accepts queries of at most 256 bytes, searches at most 100 newest records already filtered to the exact current GitLab instance and project, and returns at most five matches per call. A review permits at most eight memory calls independently of the eight internal repository and eight public-source category ceilings, with at most 16 calls combined across all tools and a one-second deadline for each memory query. Tool results are limited to 256 KiB in aggregate per review and 64 KiB each, with narrower listing, read, search, path, URL, and query limits owned by application code.
