...
An authenticated `Note Hook` create or update for an authorized, non-system, non-internal merge request comment is eligible when that merge request already has a completed review revision whose effective canonical review has both a durable publication and a locally validated structured result. Select the most recently created eligible completed revision job. A direct revision resolves to itself; an equivalent revision resolves through its canonical job. An external publication recovered after local state loss is not eligible because Wormtamer cannot safely reconstruct that result from rendered Markdown. In one transaction, insert a delivery-deduplicated feedback event containing only bounded source identifiers and create or update one feedback job per GitLab note. A note's first eligible event binds the job to that exact effective canonical review; later updates retain the immutable target. Do not persist the note body. An event without an eligible locally backed review is retained as ignored. Source timestamps use UTC whole-second precision because GitLab Note Hooks omit fractional seconds. A distinct delivery in the same source second requeues evaluation rather than being treated as stale; only an earlier source second is stale. An accepted update immediately deactivates prior derived memory.

Every eligible comment is evaluated, including comments bound to zero-finding reviews and ordinary discussion that may produce no structured decision. Feedback jobs fetch the current note after webhook commit, so processing intentionally evaluates the latest GitLab text rather than preserving comment revisions. Each job has at most five claims, a renewable three-minute lease, and the same retry categories and bounded exponential backoff used by review work. The structured current evaluation and active memory replace prior derived state transactionally. Memory versions also use whole-second timestamps; a replacement that would reuse the existing version timestamp rolls back without deleting the prior memory and retries through the normal backoff. A crash may repeat GitLab reads and Gemini evaluation but cannot create duplicate current memory.
...
An available patch ID suppresses work only when the newest canonical job by revision-job creation order in the same GitLab instance, project, and merge request is completed with its own local result and durable publication. The new exact-head job records the patch ID and canonical job relationship and completes without a result, findings, memory-retrieval audit, publication, Gemini call, or repository archive. Canonical jobs cannot point to another canonical job, so aliases never form chains.
...
Before each generation, the review loop declares only tool categories with remaining category and combined call capacity. It prevalidates every requested name before broker dispatch, then processes a valid same-turn batch in model response order. Calls admitted within both budgets are charged and dispatched sequentially; each excess call is not dispatched and receives an ordered response containing only `repository_tool_call_limit_exceeded`, `memory_tool_call_limit_exceeded`, `public_source_tool_call_limit_exceeded`, or `tool_call_limit_exceeded`. A category-specific limit takes precedence when both it and the combined limit apply. These fixed denial responses are protocol bookkeeping, not tool evidence, and do not consume aggregate tool-result bytes. A denied call makes the following generation final-only even if another category has remaining capacity; admitting the sixteenth combined call does the same without requiring an excess request. Final-only generation declares no tools, sets function-calling mode to `NONE`, and gets one opportunity within the current review attempt to return the locally validated structured result. A function call, incomplete finish, malformed content, or invalid structured result in final-only mode is not executed or corrected in place and retains the established review retry behavior.

Info generation logs expose per-turn `tool_call_count` and bounded `tool_names` for production observation. For successful attempts without denied calls, `Σ max(tool_call_count − 1, 0)` is the model generations avoided versus one call per turn; these fields show batching but cannot prove semantic independence.
