...
When a valid internal repository listing, read, or search request exceeds its per-call output limit, the tool loop returns only the `repository_tool_output_limit_exceeded` category to Gemini. The failed request still consumes one internal repository and combined tool call, and Gemini may spend another call on narrower arguments. The application does not truncate the result, disclose partial content, choose narrower arguments, or retry the request itself. Other exhausted limits and broker failures retain their existing failure semantics.
