...
Repository workspaces are disposable and rebuilt when a review attempt restarts. The current GitLab broker limits requests to 30 seconds, metadata responses to 256 KiB, each diff or note page to 2 MiB, changed diffs to five pages, 100 files, 512 KiB of aggregate diff content, and repository archives to 32 MiB compressed. Archive extraction accepts at most 20,000 entries and 128 MiB uncompressed, exposing at most 10,000 UTF-8 text files of at most 2 MiB each. Repository tools permit at most eight calls and 256 KiB of aggregate results per review; each result is at most 64 KiB, with narrower listing, read, search, path, and query limits owned by application code.
