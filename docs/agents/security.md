...
Panel responses use no third-party assets or JavaScript and set a restrictive content security policy, framing denial, no-referrer policy, content-type sniffing denial, and `Cache-Control: no-store` for state pages. Collection queries have fixed page limits and validated state, active, time-window, model, project, request-kind, and cursor filters.

Build the displayed configuration from an explicit type containing only the GitLab base URL, configured Gemini endpoint and model, thinking level, log level, authorized repositories, effective sharing rules, and approved public sources. Workflow, memory, and usage views must not select raw webhook payloads, source comment bodies, stored error messages, publication markers, prompts, model responses, model conversations, tool arguments or results, repository content, logs, credentials, or filesystem paths. Generation detail exposes only bounded structured metadata and sanitized declared tool names. Usage pages show aggregate estimated cost but not per-generation cost, formulas, or pricing rates. The panel has no mutation routes and performs no external requests.
