...
Load the configured model API key from the deployment configuration and pass it only to the `google.golang.org/genai` client. The client uses the Gemini Developer API when no model base URL is configured and otherwise sends the key, prompts, tool results, and model requests to the exact operator-configured HTTP or HTTPS endpoint. Redirects are rejected. A configured endpoint is trusted with the same private review content as the direct Gemini API and must provide the required native Gemini Developer API behavior for the configured model. The explicit function-calling loop declares only currently available brokered repository, runtime-memory, and public-source read tools; application code validates, authorizes, limits, and dispatches every admitted function request itself. A budget-denied or undeclared request never reaches a broker.

Direct Gemini pricing retrieval makes one credential-free, redirect-rejecting, size- and time-bounded HTTPS request to LiteLLM's fixed public catalog URL during service startup. The configured model is matched locally and is not placed in the request URL. Catalog content is untrusted diagnostic input: only bounded non-negative rates for the exact `gemini/<configured-model>` entry with Gemini chat metadata are admitted. A custom model endpoint receives no separate pricing request; Wormtamer reads only a bounded numeric LiteLLM cost response header already returned with model traffic.
...
Model-directed public web access is installation-wide and limited to canonical domains in `public_sources.allowed_domains`. An entry authorizes the exact domain and dot-boundary subdomains; it does not authorize an unlisted redirect target. Direct requests use HTTPS on the default port, contain no user information or query string, and fetch supported UTF-8 text only. The broker ignores environment proxy configuration. The separate application-owned pricing request is restricted to the fixed credential-free LiteLLM catalog URL described under [Credentials](#credentials); repository or model content cannot influence its destination or body.
...
Panel responses use no third-party assets or JavaScript and set a restrictive content security policy, framing denial, no-referrer policy, content-type sniffing denial, and `Cache-Control: no-store` for state pages. Collection queries have fixed page or buffer limits and validated state, time-window, model, component, level, project, merge-request, request-kind, generation, and cursor filters.

Build the displayed configuration from an explicit type containing only the GitLab base URL, configured Gemini endpoint and model, thinking level, log level, authorized repositories, effective sharing rules, and approved public sources. Workflow, memory, and usage views must not select raw webhook payloads, transient diff or comment bodies, stored error messages, publication markers, prompts, model responses, model conversations, tool arguments or results, repository content, logs, credentials, or filesystem paths. Generation detail exposes only bounded structured metadata and sanitized declared tool names. Usage pages show aggregate estimated cost but not per-generation cost, formulas, or pricing rates.

Only the explicit diagnostic routes may read the process-local diagnostic buffers. At `debug`, conversation pages can expose private diffs, comments, repository and public-source excerpts, memory lessons, model output, and unknown secrets; at other log levels they receive no content fields. Known configured credentials are replaced wholesale before buffering. Diagnostic text is rendered through `html/template` in plain-text elements without Markdown, HTML interpretation, or model- or log-supplied links. Thought text, thought signatures, and SDK protocol fields are never buffered. The logs route reads the bounded structured observer, not stderr, files, container APIs, or an external logging service; content-bearing Gemini debug events contain only a conversation reference in this observer. The panel has no mutation routes and performs no external requests.
...
Durable model-generation diagnostics contain only application-owned workflow correlation, request timing, configured and resolved model labels, finish and validation outcomes, sanitized declared tool names, numeric token metadata, completion state, and an optional cost source and integer USD estimate. The raw HTTP response body and headers are not retained; trusted code reads only the SDK's processed LiteLLM cost header when present. Prompts, candidates, thought signatures, tool arguments, tool results, comments, memory lessons, repository content, credentials, and logs are excluded.

Log operational identifiers and outcomes rather than repository content. Webhook logs use bounded delivery, project, merge request, revision, job, and outcome fields when available, plus a sanitized rejection or failure reason. Successful internal repository tool calls log the exact repository identifier, tool name, turn, and completed outcome at `info`; file paths, queries, arguments, and results remain excluded. Never log webhook bodies, request headers, credentials, webhook secrets, authorization headers, or unnecessary private source. Diagnostic content logging is explicitly enabled only by `log_level: "debug"`; stderr records model system instructions, prompts, validated responses, and tool-call arguments and results. The process-local conversation buffer records the corresponding application-visible sequence without thoughts or SDK protocol data, while the panel log buffer stores only references for those content-bearing events. Known configured credentials are replaced wholesale before diagnostic buffering and stderr diagnostic values use the same rule, but private source and secrets unknown to Wormtamer may still appear. Protect debug logs and the diagnostic panel as sensitive data, enable debug only while diagnosing, and return to `info` afterward.
