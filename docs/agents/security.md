...
Optional [CI eligibility](reliability.md#pipeline-association) reads bounded REST MR metadata and, for generated-commit pipelines, confirms the current-head association through a fixed GraphQL query. These are trusted broker reads, never model tools or model context.
