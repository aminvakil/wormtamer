...
Panel responses use no third-party assets or JavaScript and set a restrictive content security policy, framing denial, no-referrer policy, content-type sniffing denial, and `Cache-Control: no-store` for state pages. Collection queries have fixed page limits and validated state, time-window, model, project, request-kind, and cursor filters.
