...
Structured JSON stderr is the sole source of content-bearing model diagnostics. At `debug`, the review and feedback paths log their application-visible system instructions, prompts, accepted responses, tool calls and admitted results, and validated decisions. They retain no separate diagnostic conversation or log state in the process. Durable model-generation records remain a content-free usage subsystem described above and are not populated through a diagnostic observer.
