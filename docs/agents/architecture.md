...
Structured JSON stderr is the sole source of model diagnostics. At `debug`, the review and feedback paths log their application-visible system instructions, prompts, accepted responses, tool calls and admitted results, and validated decisions. They retain no separate diagnostic conversation or log state in the process or SQLite.
...
The built-in panel provides server-rendered HTML at `GET /`, with bounded history and detail views at `GET /reviews`, `GET /reviews/{job-id}`, `GET /feedback`, `GET /feedback/{job-id}`, and `GET /memory`. It shows committed review and feedback job state, patch-ID availability, validated review results and finding identities, publication status, memory-retrieval identities, feedback-derived memory, and an explicit non-secret configuration summary including the effective current-only or all-authorized sharing mode. Equivalent jobs link to their canonical review instead of presenting its result, findings, publication, or feedback synthesis as their own. A publication recovered externally without a local result is labeled external-only rather than reconstructed. A reconciled job without an associated webhook path is shown by numeric project ID.

Panel handlers query SQLite through fixed-size cursor pagination and bounded aggregate groups. No panel handler makes GitLab, Gemini, repository, file, container, or logging-service requests. The panel exposes no state-changing methods and cannot retry work, create reviews, change logging, or edit configuration or memory. It requires no presentation-only persistent state. Panel access and traffic controls deliberately remain at the deployment boundary described in [Security](security.md#read-only-web-panel).
