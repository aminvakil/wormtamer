...
3. [Evaluate review generation settings on current merge requests](evaluate-review-generation-settings.md): compare default and high thinking on the observed MRs and raise the generation ceiling only when measured truncation requires it.
4. [Enable high thinking for merge request reviews](enable-high-thinking-for-reviews.md): apply the evaluated high-thinking setting and smallest sufficient bounded generation budget.
